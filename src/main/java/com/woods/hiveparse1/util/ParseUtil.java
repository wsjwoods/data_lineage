...
import com.woods.hiveparse1.bean.DWTask;
import com.woods.hiveparse1.exception.SQLExtractException;
...
public class ParseUtil {
    ...
    private static final Map<Integer, String> hardcodeScriptMap = new HashMap<>();
    ...
    public static String collectionToString(Collection<String> coll, String split, boolean isCheck) {
        ...
        if (!Check.isEmpty(coll)) {
            for (String string : coll) {
                if (!isCheck || !Check.isEmpty(string)) {
    ...
    public static List<String> extractSQL(DWTask task) {
        ...
        if (path.endsWith("sh")) { //匹配shell
            ...
            for (String sqlVar : sqlVarList) {
                ...
                String _sql = sql;
                if (!Check.isEmpty(sql) && sql.toLowerCase().contains("insert") && sql.toLowerCase().contains("table")) {
    ...
    private static List<String> fixSqlVarList(String content, List<String> sqlList) {
        List<String> list = new ArrayList<>(sqlList.size());
        ...
        if (!Check.isEmpty(varSet)) {
            for (String var : varSet) {
                ...
                if (isVarAlone) {
                    ...
                    if (!Check.isEmpty(matchShellVar(_tsql, false))) { //防止变量套变量的情况
    ...
    public static Map<String, List<ColLine>> cloneSubQueryMap(Map<String, List<ColLine>> map) {
        Map<String, List<ColLine>> map2 = new HashMap<>(map.size());
        for (Entry<String, List<ColLine>> entry : map.entrySet()) {
            List<ColLine> value = entry.getValue();
            List<ColLine> list = new ArrayList<>(value.size());
    ...
    public static List<ColLine> cloneList(List<ColLine> list) {
        List<ColLine> list2 = new ArrayList<>(list.size());
