...
//                    nowQueryTable = nowQueryTable
    ...
    public static void main(String[] args) throws IOException, ParseException,
        ...
        String sql24 = "select  ID,\n" +
                "       RECODE,\n" +
                "       OUTER_ORDER_ID,\n" +
                "       a.CLIENT_ID,\n" +
                "       max(b.customer_code) customer_code,\n" +
                "       OPERATING_ITEM,\n" +
                "       a.VOUCHER_NUMBER,\n" +
                "       VOUCHER_TYPE,\n" +
                "       MEMBER_TYPE,\n" +
                ...
                "       a.BBH  bbh,\n" +
                "       ''  tdate\n" +
                "  from t_voucher_transaction_order a\n" +
                "  left join  t_exchange_voucher_config b\n" +
                "    on a.CLIENT_ID=b.client_id\n" +
                "\tleft join\n" +
                "\t(\n" +
                "\t\tSELECT\n" +
                "\t\t\tog.voucher_no VOUCHER_NUMBER,\n" +
                "\t\t\tmax(om.platform_id) PLATFORM_ID,\n" +
                "\t\t\tmax(fopd.card_no) CARD_NO\n" +
                "\t\tFROM\n" +
                "\t\t\tapp_order_goods og\n" +
                "\t\tJOIN app_order_sub os ON os.id = og.order_sub_id\n" +
                "\t\tJOIN app_order_master om ON os.order_master_id = om.id\n" +
                "\t\tLEFT JOIN app_fin_master_order_pay fmop ON om.id = fmop.master_order_id\n" +
                "\t\tLEFT JOIN app_fin_order_pay_detail fopd ON fopd.order_pay_id = fmop.id\n" +
                "\t\tWHERE\n" +
                "\t\tos.order_type_id = 2\n" +
                "\t\tAND om.status > 110\n" +
                "    and og.voucher_no is not null\n" +
                "\t\tgroup by og.voucher_no\n" +
                "\t) c\n" +
                "\ton a.VOUCHER_NUMBER=c.VOUCHER_NUMBER\n" +
                " where a.RECODE=0\n" +
                " group by ID,\n" +
                "       RECODE,\n" +
                "       OUTER_ORDER_ID,\n" +
